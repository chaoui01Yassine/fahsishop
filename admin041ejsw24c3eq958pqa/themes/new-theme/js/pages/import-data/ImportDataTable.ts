...
export default class ImportDataTable {
   ...
   */
  importPreviousPageHandler(): void {
   ...
   * @private
   */
  private importPaginationHandler(direction: string): void {
    const $currentPageElements = $importDataTable.find('th:visible,td:visible');
    const $oppositePaginationButton = direction === FORWARD
      ? $('.js-import-next-page')
      : $('.js-import-previous-page');
    ...
    for (const index in $tableColumns) {
      ...
      if (lastVisibleColumnFound) {
        ...
        const showColumnIndex = direction === BACKWARD
          ? this.totalNumberOfColumns - 1 - <number>(<unknown>index)
          : index;
        this.showTableColumnByIndex(<number>showColumnIndex);
   ...
   * @private
   */
  private getNumberOfVisibleColumns(): number {
   ...
   * @private
   */
  private getTotalNumberOfColumns(): number {
   ...
   * @private
   */
  private hide($elements: JQuery): void {
   ...
   * @private
   */
  private show($elements: JQuery): void {
   ...
   * @private
   */
  private showTableColumnByIndex(columnIndex: number): void {
    ...
    this.show(
      $importDataTable.find('tbody > tr').find(`td:nth-child(${colIndex})`),
    );
  }
}
