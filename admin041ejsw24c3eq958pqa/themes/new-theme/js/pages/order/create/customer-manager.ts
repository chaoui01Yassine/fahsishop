...
export default class CustomerManager {
   ...
   * @private
   */
  private initAddCustomerIframe(): void {
  ...
  private onCustomerSelect(): void {
    EventEmitter.on(eventMap.customerSelected, (event) => {
      ...
      const createAddressUrl = this.router.generate('admin_addresses_create', {
        liteDisplaying: 1,
        submitFormAjax: 1,
  ...
  loadCustomerCarts(currentCartId: string): void {
    ...
    $.get(this.router.generate('admin_customers_carts', {customerId}))
      .then((response) => {
  ...
  loadCustomerOrders(): void {
    ...
    $.get(this.router.generate('admin_customers_orders', {customerId}))
      .then((response) => {
   ...
   * @return {Number}
   */
  selectCustomer(chooseCustomerEvent: JQueryEventObject): number {
  ...
  search(searchPhrase: string): void {
    ...
    const $searchRequest = $.get(
      this.router.generate('admin_customers_search'),
      {
