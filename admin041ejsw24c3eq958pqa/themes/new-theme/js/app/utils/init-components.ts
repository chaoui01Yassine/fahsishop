    ...
    initComponents(components: string[]) {
      components.forEach((component: string): void => {
        if (window.prestashop.component[component] === undefined) {
          console.error(`Failed to initialize fahsishop component "${component}". This component doesn't exist.`);
        ...
        if (window.prestashop.instance[componentInstanceName] !== undefined) {
          console.warn(
            `Failed to initialize fahsishop component "${component}". This component is already initialized.`,
          );
