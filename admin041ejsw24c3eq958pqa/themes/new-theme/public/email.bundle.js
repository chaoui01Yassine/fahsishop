 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
class T{extend(t){t.getHeaderContainer().on("click",j,(()=>{window.location.reload()}))}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$}=window;class S{extend(t){t.getHeaderContainer().on("click",u.showQuery,(()=>this.onShowSqlQueryClick(t))),t.getHeaderContainer().on("click",u.exportQuery,(()=>this.onExportSqlManagerClick(t)))}onShowSqlQueryClick(t){const e=$(u.showModalForm(t.getId()));this.fillExportForm(e,t);const n=$(u.showModalGrid(t.getId()));n.modal("show"),n.on("click",x,(()=>e.submit()))}onExportSqlManagerClick(t){const e=$(u.showModalForm(t.getId()));this.fillExportForm(e,t),e.submit()}fillExportForm(t,e){const n=e.getContainer().find(f).data("query");t.find('textarea[name="sql"]').val(n),t.find('input[name="name"]').val(this.getNameFromBreadcrumb())}getNameFromBreadcrumb(){const t=$(w).find(v);let e="";return t.each(((t,n)=>{const o=$(n),i=o.find("a").length>0?o.find("a").text():o.text();e.length>0&&(e=e.concat(" > ")),e=e.concat(i)})),e}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:It}=window;class Ft{constructor(t){this.onClick=t}extend(t){this.initRowLinks(t),this.initConfirmableActions(t)}initConfirmableActions(t){t.getContainer().on("click",h.linkRowAction,(t=>{const e=It(t.currentTarget).data("confirm-message");e.length&&!window.confirm(e)&&t.preventDefault()}))}initRowLinks(t){const e=this.onClick;It("tr",t.getContainer()).each((function(){const t=It(this);It(h.linkRowActionClickableFirst,t).each((function(){const n=It(this),o=n.closest("td"),i=It(h.clickableTd,t).not(o);let s=!1;i.addClass("cursor-pointer").mousedown((()=>{It(window).mousemove((()=>{s=!0,It(window).unbind("mousemove")}))})),i.mouseup((()=>{const t=s;if(s=!1,It(window).unbind("mousemove"),!t){const t=n.data("confirm-message");(!t.length||window.confirm(t)&&n.attr("href"))&&(Y(e)||Y(n.get(0))?document.location.href=n.attr("href"):e(n.get(0)))}}))}))}))}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Dt}=window;Dt((()=>{const t=new M("email_logs");t.addExtension(new T),t.addExtension(new S),t.addExtension(new P),t.addExtension(new q),t.addExtension(new z),t.addExtension(new Mt),t.addExtension(new $t),t.addExtension(new Lt),t.addExtension(new Ft),t.addExtension(new Pt),new e,new s,new l}))})(),window.email=o})();