 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
class B{extend(t){const e=t.getContainer().find(f);new j(e).attach()}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:E}=window,A=function(t,e){E.post(t).then((()=>window.location.assign(e)))},{$:S}=window;class M{extend(t){t.getContainer().on("click",p,(t=>{A(S(t.currentTarget).data("url"),S(t.currentTarget).data("redirect"))}))}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
class P{extend(t){t.getHeaderContainer().on("click",D,(()=>{window.location.reload()}))}}var L=n(9567),$=Object.defineProperty,R=Object.getOwnPropertySymbols,I=Object.prototype.hasOwnProperty,z=Object.prototype.propertyIsEnumerable,H=(t,e,n)=>e in t?$(t,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):t[e]=n,F=(t,e)=>{for(var n in e||(e={}))I.call(e,n)&&H(t,n,e[n]);if(R)for(var n of R(e))z.call(e,n)&&H(t,n,e[n]);return t};
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Tt}=window;class jt{extend(t){t.getHeaderContainer().on("click",e.showQuery,(()=>this.onShowSqlQueryClick(t))),t.getHeaderContainer().on("click",e.exportQuery,(()=>this.onExportSqlManagerClick(t)))}onShowSqlQueryClick(t){const n=Tt(e.showModalForm(t.getId()));this.fillExportForm(n,t);const o=Tt(e.showModalGrid(t.getId()));o.modal("show"),o.on("click",O,(()=>n.submit()))}onExportSqlManagerClick(t){const n=Tt(e.showModalForm(t.getId()));this.fillExportForm(n,t),n.submit()}fillExportForm(t,e){const n=e.getContainer().find(r).data("query");t.find('textarea[name="sql"]').val(n),t.find('input[name="name"]').val(this.getNameFromBreadcrumb())}getNameFromBreadcrumb(){const t=Tt(m).find(b);let e="";return t.each(((t,n)=>{const o=Tt(n),i=o.find("a").length>0?o.find("a").text():o.text();e.length>0&&(e=e.concat(" > ")),e=e.concat(i)})),e}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
class Bt{extend(t){const e=t.getContainer().find(g);e.find(v).prop("disabled",!0),e.find(y).on("input dp.change",(()=>{e.find(v).prop("disabled",!1),e.find(w).prop("hidden",!1)}))}}n(3867);
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Et}=window;class At{constructor(t){this.grid=t}extend(t){this.grid=t,this.addIdsToGridTableRows(),t.getContainer().find(r).tableDnD({onDragClass:C,dragHandle:s,onDrop:(t,e)=>this.handlePositionChange(e)}),t.getContainer().find(".js-drag-handle").hover((function(){Et(this).closest("tr").addClass("hover")}),(function(){Et(this).closest("tr").removeClass("hover")})),this.setReorderButtonLabel(),this.getReorderButton().on("click",(t=>this.oncClickReorderButton(t)))}handlePositionChange(t){const e=Et(t).find(u(this.grid.getId())),n=e.data("update-url"),o=e.data("update-method"),i={positions:this.getRowsPositions()};this.updatePosition(n,i,o)}getRowsPositions(){const t=JSON.parse(Et.tableDnD.jsonize())[`${this.grid.getId()}_grid_table`],e=[];let n;for(let o=0;o<t.length;o+=1)n=this.grid.getContainer().find(`#${t[o]}`),e.push({rowMarker:t[o],offset:n.data("dragAndDropOffset")});return this.computeMappingBetweenOldAndNewPositions(e)}addIdsToGridTableRows(){let t=0;this.grid.getContainer().find(h(this.grid.getId())).each(((e,n)=>{const o=Et(n),i=`row_${o.data("id")}_${o.data("position")}`;o.closest("tr").attr("id",i),o.closest("td").addClass(a),o.closest("tr").data("dragAndDropOffset",t),t+=1}))}updatePosition(t,e,n){const o=["GET","POST"].includes(n),i=Et("<form>",{action:t,method:o?n:"POST"}).appendTo("body"),r=e.positions.length;let s;for(let t=0;t<r;t+=1)s=e.positions[t],i.append(Et("<input>",{type:"hidden",name:`positions[${t}][rowId]`,value:s.rowId}),Et("<input>",{type:"hidden",name:`positions[${t}][oldPosition]`,value:s.oldPosition}),Et("<input>",{type:"hidden",name:`positions[${t}][newPosition]`,value:s.newPosition}));o||i.append(Et("<input>",{type:"hidden",name:"_method",value:n})),i.submit()}computeMappingBetweenOldAndNewPositions(t){var e;const n=new RegExp("^row_(?<rowId>\\d+)_(?<oldPosition>\\d+)$"),o=[];for(let i=0;i<t.length;i+=1){const r=n.exec(t[i].rowMarker);if(r&&!G(r.groups)&&!G(r.groups.rowId)&&!G(r.groups.oldPosition)){const t=parseInt(null==(e=null==r?void 0:r.groups)?void 0:e.oldPosition,10);o[i]={rowId:r.groups.rowId,oldPosition:t,newPosition:t}}for(let e=0;e<t.length;e+=1)G(t[e])||G(t[e].offset)||G(o[t[e].offset])||G(o[e])||(o[t[e].offset].newPosition=o[e].oldPosition)}return o}isPositionsReorderActive(){return this.grid.getContainer().find('.ps-sortable-column[data-sort-col-name="position"]').first().data("sort-is-current")}getReorderButton(){return this.grid.getContainer().find(".js-btn-reorder-positions").first()}setReorderButtonLabel(){const t=this.getReorderButton(),e=this.isPositionsReorderActive()?t.data("label-save"):t.data("label-reorder");t.html(e)}oncClickReorderButton(t){t.preventDefault(),this.isPositionsReorderActive()?this.grid.getContainer().find(".ps-sortable-column").first().click():this.grid.getContainer().find('.ps-sortable-column[data-sort-col-name="position"]').first().click()}}
/**
 * Copyright since 2007 fahsishop and Contributors
 * fahsishop is an International Registered Trademark & Property of fahsishop
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:St}=window;St((()=>{const t=new x("attribute");t.addExtension(new jt),t.addExtension(new P),t.addExtension(new B),t.addExtension(new M),t.addExtension(new _t),t.addExtension(new Ot),t.addExtension(new xt),t.addExtension(new Bt),t.addExtension(new At(t))}))})(),window.attribute=o})();